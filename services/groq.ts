...
THE BOLD RULE: Do not make text bold,If you use ** or ###, you have failed the mission.
...
const fetchIslamGPTContext = async (): Promise<string> => {
  try {
    const res = await fetch("https://api.islamgpt.example.com/context", {
      method: "GET",
    });
    if (!res.ok) return "";
...
export const getGroqResponse = async (
  ...
  throw lastError || new Error("Groq models are currently unavailable");
};
